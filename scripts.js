		...
		function sprawdz(nr){
			...
			if(((licznik%10) >= 2 && (licznik%10 <= 4)) && ((licznik < 5) || (licznik > 21))){
				...
				wysLicznik.innerHTML = ("Straci³eœ wszystkie monety!");
			}
			else{
		...
		function koniecGry(odgadniete){	
				if(odgadniete == iloscLiczbDoTrafienia || odgadniete == 0){
					...
					if(odgadniete == 0){
					alert("Tak ³atwo siê poddajesz? Popatrz jakie to by³o proste i spróbuj jeszcze raz :)");
					}
					else{
						if(licznik <= 0 ){
							alert("Co za pech! Straci³eœ wszystkie monety.");
						}
						else{
							alert("Gratulacje! Zachowa³eœ " + licznik + "/" + licznikStaly + " monet");			
						}
					}
				}
		}
		function potwierdzKoniecGry() {
			var r = confirm("Czy na pewno chcesz siê poddaæ?");
			if (r == true) {
				koniecGry(0);
			}
		}
